...
.. attention::
   ...
   You have to pull the ansible repository to access the files mentioned below. You can pull the project with the
...
- Create all files needed for Openshift to deploy the new version. You can find a template in the openshift directory in the ansible repository.
  Replace **${VERSION}** with the version number without all characters which aren't alphanumeric (e.g. 2.18 -> `218`).
